...
def compute_anomaly(results, params, seq_threshold=0.4):
...
def get_predicition_with_threshold(test_normal_results, params, seq_th=0.4):
    FP = compute_anomaly(test_normal_results, params, seq_th)
    result = FP > 0
    # TP = 0 # compute_anomaly(test_abnormal_results, params, seq_th)

    # TN = len(test_normal_results) - FP
    # FN = 0 #len(test_abnormal_results) - TP
    # P = 100 * TP / (TP + FP)
    # R = 100 * TP / (TP + FN)
    # F1 = 2 * P * R / (P + R)

    # result = [0, seq_th, FP, TP, TN, FN, P, R, F1]
    return result
...
class ReqPredictor():
    ...
    def predict(self):
        ...
        # test_abnormal_results, test_abnormal_errors = self.helper(model, self.output_dir, "test_abnormal_req", vocab, scale, error_dict)
